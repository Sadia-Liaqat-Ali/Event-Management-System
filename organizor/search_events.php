...
if (isset($_GET['search']) && !empty(trim($_GET['search']))) {
    ...
    if (preg_match('/^\d{4}-\d{2}-\d{2}$/', $searchQuery)) {
        $stmt = $pdo->prepare("SELECT * FROM events WHERE organizer_id = ? AND event_date = ?");
...
<body>
    ...
    <div class="container">
        ...
        <div class="alert alert-info">
            <strong>Note:</strong> Please enter the event date in the format YYYY-MM-DD to search for your events.Then you are able to view all the events available for this specific date.
        </div>
                    ...
                    <div class="col-md-4 mb-4">
                        <div class="card">
                            ...
                            <div class="card-footer d-flex justify-content-between">
                                ...
                                <a href="search_event.php?delete=<?php echo $event['id']; ?>" class="btn btn-danger btn-action" onclick="return confirm('Are you sure you want to delete this event?');"><i class="fas fa-trash-alt"></i> Delete</a>
                            </div>
                        </div>
                    </div>
                <?php endforeach; ?>
